...
// MEDIA
// ============================================================
// Video model configs for UI switching
const _videoModels = new Set(["ltx2", "wan-t2v"]);
const _fallbackModelDefaults = {
  "flux2-klein": { steps: 4, guidance: 4.0, width: 1024, height: 1024 },
  sdxl: { steps: 18, guidance: 6.5, width: 768, height: 768 },
  "sdxl-turbo": { steps: 6, guidance: 0.0, width: 768, height: 768 },
  flux: { steps: 20, guidance: 5.5, width: 768, height: 768 },
  "pony-xl-v6": { steps: 25, guidance: 7.0, width: 1024, height: 1024 },
  "wai-illustrious-xl": { steps: 20, guidance: 6.0, width: 1024, height: 1024 },
  "unholy-desire-v7": { steps: 20, guidance: 3.5, width: 1024, height: 1024 },
  "z-image-fp8": { steps: 10, guidance: 1.0, width: 768, height: 768 },
  "z-image-q8-gguf": { steps: 32, guidance: 4.0, width: 768, height: 768 },
  easydiffusion: { steps: 28, guidance: 7.0, width: 768, height: 768 },
  perchance: { steps: 20, guidance: 7.0, width: 768, height: 768 },
  perchance_other: { steps: 20, guidance: 7.0, width: 768, height: 768 },
  playground: { steps: 20, guidance: 5.5, width: 768, height: 768 },
  pixart: { steps: 20, guidance: 5.5, width: 768, height: 768 },
  "wan-t2v": {
    steps: 30,
    guidance: 7.5,
    width: 480,
...
let _modelDefaults = { ..._fallbackModelDefaults };

function _setMediaFieldValue(id, value) {
  const el = document.getElementById(id);
  if (!el || value == null) return;
  el.value = value;
}

function _getSelectedMediaModelLabel() {
  return (
    document.getElementById("media-model")?.selectedOptions?.[0]?.textContent?.trim() ||
    "selected model"
  );
}

function _applyMediaModelDefaults(model) {
  const defaults = _modelDefaults[model];
  if (!defaults) return;
  _setMediaFieldValue("media-steps", defaults.steps);
  _setMediaFieldValue("media-guidance", defaults.guidance);
  _setMediaFieldValue("media-width", defaults.width);
  _setMediaFieldValue("media-height", defaults.height);
  _setMediaFieldValue("media-frames", defaults.frames);
  _setMediaFieldValue("media-fps", defaults.fps);
  _setMediaFieldValue("media-epoch", defaults.epoch);
}

function _syncMediaParameterUi(model) {
  const defaults = _modelDefaults[model];
  const keepDefaults =
    document.getElementById("media-use-model-defaults")?.checked ?? true;
  const stepsInput = document.getElementById("media-steps");
  const guidanceInput = document.getElementById("media-guidance");
  const hintEl = document.getElementById("media-parameter-hint");
  const modelLabel = _getSelectedMediaModelLabel();

  if (stepsInput) {
    stepsInput.disabled = keepDefaults;
    stepsInput.title =
      keepDefaults && defaults
        ? `${modelLabel} will use its recommended step count.`
        : "";
  }
  if (guidanceInput) {
    guidanceInput.disabled = keepDefaults;
    guidanceInput.title =
      keepDefaults && defaults
        ? `${modelLabel} will use its recommended guidance scale.`
        : "";
  }
  if (!hintEl) return;
  if (!defaults) {
    hintEl.textContent = keepDefaults
      ? "Using built-in defaults for the selected model."
      : "";
    return;
  }
  hintEl.textContent = keepDefaults
    ? `Using recommended settings for ${modelLabel}: ${defaults.steps} steps, guidance ${defaults.guidance}. Uncheck the box to override them manually.`
    : `Recommended for ${modelLabel}: ${defaults.steps} steps, guidance ${defaults.guidance}.`;
}

async function loadMediaModelDefaults() {
  try {
    const { res, data } = await fetchJsonSafe("/media/models");
    if (!res.ok) {
      throw new Error(data?.detail || `HTTP ${res.status}`);
    }
    const nextDefaults = { ..._fallbackModelDefaults };
    (data.models || []).forEach((model) => {
      const existing = nextDefaults[model.model_key] || {};
      const defaults = model.defaults || {};
      nextDefaults[model.model_key] = {
        ...existing,
        steps: defaults.num_inference_steps ?? existing.steps,
        guidance: defaults.guidance_scale ?? existing.guidance,
        width: defaults.width ?? existing.width,
        height: defaults.height ?? existing.height,
        frames: defaults.num_frames ?? existing.frames,
        fps: defaults.fps ?? existing.fps,
        epoch: defaults.epoch ?? existing.epoch,
      };
    });
    _modelDefaults = nextDefaults;
    _applyMediaModelDefaults(
      document.getElementById("media-model")?.value || "flux2-klein",
    );
    _syncMediaParameterUi(
      document.getElementById("media-model")?.value || "flux2-klein",
    );
  } catch (e) {
    console.warn("loadMediaModelDefaults:", e);
  }
}

function _onMediaDefaultsModeChange() {
  const model = document.getElementById("media-model")?.value || "";
  const keepDefaults =
    document.getElementById("media-use-model-defaults")?.checked ?? true;
  if (keepDefaults) _applyMediaModelDefaults(model);
  _syncMediaParameterUi(model);
}

function _onMediaModelChange() {
  const model = document.getElementById("media-model")?.value || "";
  const isVideo = _videoModels.has(model);
  const isWan = model === "wan-t2v";
  // Show/hide video-specific fields
  const epochGroup = document.getElementById("media-epoch-group");
  const framesGroup = document.getElementById("media-frames-group");
  const fpsGroup = document.getElementById("media-fps-group");
  if (epochGroup) epochGroup.style.display = isWan ? "" : "none";
  if (framesGroup) framesGroup.style.display = isVideo ? "" : "none";
  if (fpsGroup) fpsGroup.style.display = isVideo ? "" : "none";
  // Apply model defaults
  if (_modelDefaults[model]) {
    _applyMediaModelDefaults(model);
  } else if (!isVideo) {
    // Reset to image defaults
    document.getElementById("media-steps").value = 30;
    document.getElementById("media-guidance").value = 7.5;
    document.getElementById("media-width").value = 1024;
    document.getElementById("media-height").value = 1024;
  }
  _syncMediaParameterUi(model);
  // Update button text
  const btn = document.getElementById("btn-generate-image");
  if (btn) btn.textContent = isVideo ? "Generate Video" : "Generate Image";
}
...
async function generateImage() {
  ...
  const userValue = document.getElementById("media-user-id")?.value || "";
  const userId = userValue ? Number(userValue) : undefined;
  if (userValue && (!Number.isFinite(userId) || userId <= 0)) {
    showToast("Select a valid owner or leave it blank", "warning");
    return;
  }
  ...
  try {
      ...
      statusEl.innerHTML = `
        <div style="margin-top:8px;">
          <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px;">
            <span style="color:#f59e0b;">Starting generation...</span>
          </div>
          <div class="bar-track" style="max-width:100%;height:10px;border-radius:5px;">
            <div id="media-progress-fill" class="bar-fill amber" style="width:0%;height:100%;border-radius:5px;transition:width 0.5s ease;"></div>
          </div>
          <div id="media-progress-text" class="text-xs text-muted" style="margin-top:4px;">Initializing...</div>
        </div>`;
    }
    const modelVal = document.getElementById("media-model")?.value;
    const isVideo = _videoModels.has(modelVal);
    const body = {
      prompt,
      user_id: userId,
      model: modelVal,
      negative_prompt:
        document.getElementById("media-negative")?.value || undefined,
      ...
      guidance_scale: parseFloat(
        document.getElementById("media-guidance")?.value,
      ),
      seed: document.getElementById("media-seed")?.value
        ? parseInt(document.getElementById("media-seed").value)
        : undefined,
    };
    if (isVideo) {
      body.num_frames =
        parseInt(document.getElementById("media-frames")?.value) || 33;
      body.fps = parseInt(document.getElementById("media-fps")?.value) || 16;
    }
    if (modelVal === "wan-t2v") {
      body.epoch =
        parseInt(document.getElementById("media-epoch")?.value) || 10;
    }
    ...
    if (!res.ok) {
      if (statusEl) {
        statusEl.innerHTML = `<span style="color:#ef4444;">Error: ${escapeHtml(data.detail || "Failed")}</span>`;
      }
      return;
    }
    // Poll for progress
    const jobId = data.job_id;
    const estSeconds = data.est_seconds || 45;
    const startTime = Date.now();
    const poll = async () => {
      try {
        const sr = await fetch(`/media/generate/${jobId}/status`);
        ...
        if (sd.status === "completed") {
          ...
          if (text) text.textContent = sd.detail || "Complete!";
          if (statusEl) {
            const genMs = sd.result?.generation_time_ms;
            const timeStr = genMs
              ? `${(genMs / 1000).toFixed(1)}s`
              : `${elapsed}s`;
            setTimeout(() => {
              statusEl.innerHTML = `<span style="color:#10b981;">Generated in ${timeStr}</span>`;
            }, 1500);
          }
          showToast(
            isVideo ? "Video generated!" : "Image generated!",
            "success",
          );
          loadMediaHistory();
          refreshVRAM();
        ...
        if (sd.status === "error") {
          ...
          if (text) text.textContent = sd.detail || "Failed";
          if (statusEl) {
            setTimeout(() => {
              statusEl.innerHTML = `<span style="color:#ef4444;">${escapeHtml(sd.detail || "Generation failed")}</span>`;
            }, 1500);
          }
          if (btn) btn.disabled = false;
          return;
        }
        // In progress — animate bar
        let pct = sd.progress || 0;
        // Smooth fake progress based on elapsed time if server hasn't updated
        if (pct < 90) {
          const timePct = Math.min(85, (elapsed / estSeconds) * 80);
          pct = Math.max(pct, timePct);
        }
        if (fill) {
          fill.style.width = `${pct}%`;
        }
        if (text)
          text.textContent = `${sd.detail || "Working..."} (${elapsed}s elapsed)`;
        setTimeout(poll, 1500);
      } catch (e) {
        if (statusEl)
          statusEl.innerHTML = `<span style="color:#ef4444;">Poll error: ${escapeHtml(e.message)}</span>`;
        if (btn) btn.disabled = false;
      }
    };
...
async function refreshVRAM() {
  try {
    ...
    const data = await res.json();
    const el = document.getElementById("media-vram");
    if (!el) return;
    if (data.available) {
      const runtime = data.runtime_status || {};
      const model = data.model_loaded || runtime.current_model || "None";
      const devicePercent =
        data.device_percent_used ?? data.percent_used ?? 0;
      const totalGb = data.device_total_gb ?? data.total_gb ?? 0;
      const usedGb = data.device_used_gb ?? data.allocated_gb ?? 0;
      const freeGb = data.device_free_gb ?? data.free_gb ?? 0;
      el.innerHTML = `
        <div class="metric-bar"><div class="bar-label">GPU Memory Usage</div><div class="bar-track"><div class="bar-fill ${devicePercent > 80 ? "red" : "blue"}" style="width:${devicePercent || 0}%"></div></div><div class="bar-value">${devicePercent || 0}%</div></div>
        <div class="text-xs text-muted mt-8">Device Total: ${totalGb}GB | Used: ${usedGb}GB | Free: ${freeGb}GB</div>
        <div class="text-xs text-muted">Model: ${model}</div>
        <div class="text-xs text-muted">Phase: ${runtime.phase || "unknown"} | Busy: ${runtime.busy ? "yes" : "no"} | Process-visible: ${data.process_visible ? "yes" : "no"}</div>
        <div class="text-xs text-muted">Last update: ${runtime.updated_at || "n/a"}</div>`;
    } else {
      el.innerHTML = `<div class="text-muted">CUDA not available<br>${data.message || data.error || ""}</div>`;
    }
  } catch (e) {
    console.error("refreshVRAM:", e);
  }
}
...
async function loadMediaHistory() {
  const userId = document.getElementById("media-history-user")?.value;
  try {
    const url = userId
      ? `/media/history?user_id=${userId}&limit=50`
      : "/media/history?limit=50";
    ...
    if (!data.history || data.history.length === 0) {
      el.innerHTML = '<div class="text-muted">No images generated yet</div>';
      return;
    }
    el.innerHTML = "";
    data.history.forEach((item) => {
      ...
      if (item.status === "completed" && item.file_path) {
        if (isVid) {
          mediaPreview = `<video src="/media/image/${item.id}" controls style="width:100%;max-height:240px;border-radius:6px;margin-bottom:8px;" preload="metadata"></video>`;
        } else {
          mediaPreview = `<img src="/media/image/${item.id}" alt="Generated" style="width:100%;height:200px;object-fit:cover;border-radius:6px;margin-bottom:8px;cursor:pointer;"
            onclick="window.open('/media/image/${item.id}','_blank')" onerror="this.style.display='none'" />`;
        }
      }
      card.innerHTML = `${mediaPreview}
        <div style="font-weight:600;color:${statusColor}">${statusIcon} ${item.model_used || "Unknown"}</div>
        <div class="text-xs text-muted mt-8" title="${item.prompt}">${item.prompt.substring(0, 100)}${item.prompt.length > 100 ? "..." : ""}</div>
        <div class="text-xs text-muted mt-8">${getUserDisplayName(item.user_id)} | ${formatDateTime(item.created_at)}${item.generation_time_ms ? ` | ${(item.generation_time_ms / 1000).toFixed(1)}s` : ""}</div>
        ${item.error_message ? `<div class="text-error text-xs mt-8">${item.error_message}</div>` : ""}`;
      el.appendChild(card);
    });
  } catch (e) {
    showToast(`Error: ${e.message}`, "error");
...
async function loadUserNames() {
  try {
    ...
    const targets = [
      ...
      { id: "media-history-user", blank: true, blankText: "All Users" },
...
function wireEvents() {
  ...
  // Media
  document
    .getElementById("btn-generate-image")
    ?.addEventListener("click", generateImage);
  document
    .getElementById("btn-refresh-vram")
    ?.addEventListener("click", refreshVRAM);
  document
    .getElementById("btn-unload-model")
    ?.addEventListener("click", unloadModel);
  document
    .getElementById("btn-load-history")
    ?.addEventListener("click", loadMediaHistory);
  document
    .getElementById("media-model")
    ?.addEventListener("change", _onMediaModelChange);
  document
    .getElementById("media-use-model-defaults")
    ?.addEventListener("change", _onMediaDefaultsModeChange);
  _onMediaModelChange();
  loadMediaModelDefaults();
